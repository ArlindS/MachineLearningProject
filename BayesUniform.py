'''
    ...
    Dr. Leeds
    Final Project - Bayes Classification: Uniform 
'''
...
class BayesClassifier_Uniform:
    ...
    def uniformPDF(self, x, mu, sig):
        # gets 78.49462365591397 accuracy
        return (1/sig) * ((1/(2*np.pi)) * np.exp(-(x-mu)**2/(2*sig)))
        # gets 82.79569892473118 accuracy
        # return np.sqrt(1/sig) * np.exp(-(1/sig)*((x-mu)**2)/2)
...
"""
bayesUniform = BayesClassifier_Uniform(trainData[:, 1:], trainData[:, 0])
print("Test data performance accuracy: --> ",
      bayesUniform.evaluateBayes(testData[:, 1:], testData[:, 0]))
print("Train data performance accuracy: --> ", bayesUniform.evaluateBayes(
...
filterM.computeFilterMethod(BayesClassifier_Uniform, trainData)
print("**********     Perfomance of filter method on Test Data     *********")
filterM.testFilterMethod(BayesClassifier_Uniform, testData)
"""

wrapperM = WrapperMethod(trainData)
print("\n**********     WRAPPER METHOD     **********\n")
print("**********     Perfomance of filter method on train Data     *********")
wrapperM.computerWrapperMethod(BayesClassifier_Uniform)
print("**********     Perfomance of filter method on Test Data     *********")
wrapperM.testWrapperMethod(BayesClassifier_Uniform, testData)
